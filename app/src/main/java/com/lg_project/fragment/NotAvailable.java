...
public class NotAvailable extends Fragment implements SearchView.OnQueryTextListener {
    ...
    private ArrayList<ContactorModel> arraylist = new ArrayList<>();
    ...
    int body_TypeId = 00, raceID = 00, minAge=15,maxAge=70, minWeight =100,maxWeight=250;
...
//        });
    ...
    void filter(int minAge,int maxAge,int minWeight, int maxWeight,double minHeight, double maxHeight) {
        ...
        for (ContactorModel d : arraylist) {
            ...
            if(!((minHeight == 3.5) && (maxHeight == 6.5))) {
                if (!(minHeight > 3.5 && maxHeight < 6.5) &&
                ...
                try {
                    ...
                    if(response.isSuccessful()) {
                        ...
                        for (int i = 0; i < data.size(); i++) {
                            if (data.get(i).getStatus().equals("Not_Available")) {
                                Log.e("RERERE", String.valueOf(data.get(i).getId()));
                                ContactorModel model = new ContactorModel();
                                Log.e("IIDDDDDD", String.valueOf(data.get(i).getId()));
...
//                customAdapter.notifyDataSetChanged();
    ...
    public void getBodyType() {
        ...
        call.enqueue(new Callback<ResponseBody>() {
            ...
            public void onFailure(Call<ResponseBody> call, Throwable t) {
                //  Log.d("ERRor",t.getMessage());
                Toast.makeText(getActivity(), ""+getResources().getString(R.string.something_wrong_msg), Toast.LENGTH_SHORT).show();
            }
        });
